...
Ltac get_float t :=
  let get_float_rational s n d :=
    ...
    let e := aux d in
    eval vm_compute in (F.fromF (@Basic.Float F.radix s n (Zneg e))) in
  let get_float_bpow s n d :=
    eval vm_compute in (F.fromF (@Basic.Float F.radix s n d)) in
  let get_float_integer s t :=
    ...
    let v := aux t Z0 in
    match v with
    | (?m, ?e) => eval vm_compute in (F.fromF (@Basic.Float F.radix s m e))
    end in
  match t with
  | 0%R => F.zero
...
Lemma Rabs_contains :
  ...
  match F.toF f with
  | Basic.Float false m e => (Rabs v <= FtoR F.radix false m e)%R
  | _ => True
  end.
Proof.
intros f v (H1,H2).
...
easy.
intros l u [_ Hu].
generalize (I.upper_bounded_correct (Float.Ibnd l u)).
simpl.
case F.real.
2: easy.
intros H _.
...
easy.
intros l u [Hl _].
generalize (I.lower_bounded_correct (Float.Ibnd l u)).
simpl.
case F.real.
2: easy.
intros H _.
...
case: (I.convert (Taylor_model_sharp.error (iF' iab))).
  by case.
