...
Lemma bounded_is_correctly_rounded :
  forall beta mode prec (f : float beta),
  is_bounded prec f = true ->
  is_correctly_rounded beta mode prec (FtoX f) (FtoX f).
intros until f.
case f.
intro. discriminate.
intros _.
...
case mode ; try apply Rle_refl.
intros.
case (Req_dec (FtoR beta true p z)%R g) ; intros.
left. exact H1.
right.
left.
unfold Rminus.
...
case mode ; try apply Rle_refl.
intros.
case (Req_dec (FtoR beta false p z) g) ; intros.
left. exact H1.
right.
left.
unfold Rminus.
...
Lemma zero_is_correctly_rounded :
  ...
  is_correctly_rounded beta mode prec (FtoX (Fzero beta)) (Xreal R0).
intros.
change (Xreal R0) with (FtoX (Fzero beta)).
